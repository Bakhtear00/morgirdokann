...
const SalesModule: React.FC<SalesModuleProps> = ({ sales, refresh }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.total || Number(formData.total) <= 0) {
      addToast('মোট বিক্রয়ের টাকার পরিমাণ সঠিকভাবে লিখুন!', 'error');
      return;
    }
      ...
      <div className={`bg-white p-6 md:p-8 rounded-[2rem] shadow-xl border-2 ${editingId ? 'border-orange-500' : 'border-gray-100'} no-print`}>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="col-span-1">
            ...
            <input id="sale-pieces" type="text" inputMode="numeric" pattern="[0-9]*" value={formData.pieces} onChange={(e) => setFormData({ ...formData, pieces: e.target.value })} className="w-full px-4 py-4 rounded-xl bg-white border-2 border-gray-200 text-gray-900 font-bold text-lg outline-none" placeholder="০" />
          </div>
          <div className="col-span-1">
            <label htmlFor="sale-rate" className="text-sm font-black text-gray-700 mb-1.5 block ml-1">গড় দর (ঐচ্ছিক)</label>
            <input id="sale-rate" type="text" inputMode="decimal" value={formData.rate} onChange={(e) => setFormData({ ...formData, rate: e.target.value })} className="w-full px-4 py-4 rounded-xl bg-white border-2 border-gray-200 text-gray-900 font-bold text-lg outline-none" placeholder="০" />
          </div>
          <div className="col-span-1">
            <label htmlFor="sale-mortality" className="text-sm font-black text-red-600 mb-1.5 block ml-1">মোট মৃত্যু</label>
            <input id="sale-mortality" type="text" inputMode="numeric" pattern="[0-9]*" value={formData.mortality} onChange={(e) => setFormData({ ...formData, mortality: e.target.value })} className="w-full px-4 py-4 rounded-xl bg-white border-2 border-red-200 text-gray-900 font-bold text-lg outline-none focus:border-red-600" placeholder="০" />
          </div>
          <div className="md:col-span-2 lg:col-span-3">
            <label htmlFor="sale-total" className="text-lg font-black text-green-700 mb-2 block ml-1">মোট বিক্রয় টাকা *</label>
            <input id="sale-total" type="text" inputMode="decimal" value={formData.total} onChange={(e) => setFormData({ ...formData, total: e.target.value })} className="w-full px-6 py-7 rounded-3xl bg-green-50/20 border-4 border-green-600 text-green-700 font-black text-5xl outline-none" placeholder="৳ ০" required />
          </div>
